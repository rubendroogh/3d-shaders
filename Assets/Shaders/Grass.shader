Shader "Custom/Grass"
{
	Properties
	{
		...
		_TessellationDistance("Tessellation Grass Distance", Range(0.01, 10)) = 0.1
		_TessellationDistanceStrength("Tessellation Distance Modifier", Range(0.5, 3)) = 1
	...
	SubShader
	{
		...
		CBUFFER_START(UnityPerMaterial)
			...
			float _TessellationDistanceStrength;
		...
		// into account.
		float tessellationEdgeFactor_distance(VertexInput vert0, VertexInput vert1)
		{
			...
			//return clamp(edgeLength * _ScreenParams.y / (_TessellationDistance * viewDist), _TessellationDistanceMin, _TessellationDistanceMax);
			//return max(edgeLength * _ScreenParams.y / (_TessellationDistance * viewDist), _TessellationDistanceMin);
			return edgeLength * (_ScreenParams.y * 1.1f) / (_TessellationDistance * viewDist * _TessellationDistanceStrength);
		}
		...
		TessellationFactors patchConstantFunc(InputPatch<VertexInput, 3> patch)
		{
			...
			f.edge[0] = tessellationEdgeFactor_distance(patch[1], patch[2]);
			f.edge[1] = tessellationEdgeFactor_distance(patch[2], patch[0]);
			f.edge[2] = tessellationEdgeFactor_distance(patch[0], patch[1]);
		...
		// This pass draws shadows originating form the grass blades
		//Pass
		//{
